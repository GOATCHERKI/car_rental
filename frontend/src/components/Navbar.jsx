...
const Navbar = () => {
  ...
  return (
    ...
    py-4 text-gray-600 border-b border-border relative transition-all
    ${location.pathname === "/" && "bg-light"}  `}>     
        <Link to='/'>
            <motion.img whileHover={{scale: 1.02}} src={assets.logo} alt="Logo" className='h-10'/>
        </Link>

        <div className={`max-sm:fixed max-sm:h-screen max-sm:w-full max-sm:top-16
        max-sm:border-t border-border right-0 flex flex-col sm:flex-row  items-start
        ...
        ${open ? "max-sm:translate-x-0" : "max-sm:translate-x-full"}`}>
            ...
            <div className='hidden lg:flex items-center text-sm gap-2 border border-border px-3 rounded-full max-w-56'>
            ...
            <div className='flex max-sm:flex-col items-start sm:items-center gap-6'>
                ...
                className='cursor-pointer px-8 py-2 bg-primary hover:bg-secondary transition-all text-white rounded-lg'>{user ? 'Logout' : 'Login'}</button>
            </div>
