...
const ManageBookings = () => {
  ...
  return (
    <div className='px-4 pt-10 md:px-10 w-full'>
        <Title title="Manage Bookings" subTitle="Manage all your customers bookings from here"/>
          ...
          <table className='w-full border-collapse text-left text-sm text-gray-600'>
            ...
            <tbody>
              {bookings.map((booking, index)=>(
                <tr key={index} className='border-t border-border text-gray-500'>
                  ...
                  <td className='p-3'>
                    {booking.status === 'pending' ? (
                      <select onChange={e=>changeBookingStatus(booking._id, e.target.value)} value={booking.status} className='px-2 py-1.5 mt-1 text-gray-500 border border-border rounded-md outline-none'>
